...
export class Game {
  ...
  private hoveredShape: Shape | null = null; // New hover state
  ...
  private getResizeHandles(shape: Shape): { x: number; y: number; width: number; height: number; handle: string }[] {
    ...
    const handleSize = 10; // Smaller professional handles
    ...
    if (shape.type === "rect" || shape.type === "ellipse" || shape.type === "triangle") {
      ...
      const textHeight = shape.fontSize * this.scale * 1.3; // Approx line height


      handles.push({ x: screenPos.x - handleSize / 2, y: screenPos.y - handleSize / 2, width: handleSize, height: handleSize, handle: "nw" });
      handles.push({ x: screenPos.x + textWidth - handleSize / 2, y: screenPos.y - handleSize / 2, width: handleSize, height: handleSize, handle: "ne" });
      handles.push({ x: screenPos.x - handleSize / 2, y: screenPos.y + textHeight - handleSize / 2, width: handleSize, height: handleSize, handle: "sw" });
      handles.push({ x: screenPos.x + textWidth - handleSize / 2, y: screenPos.y + textHeight - handleSize / 2, width: handleSize, height: handleSize, handle: "se" });
    }
  ...
  setFillColor(color: string) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.fillColor = color;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setStrokeColor(color: string) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.strokeColor = color;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setStrokeWidth(width: number) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.strokeWidth = width;
      this.selectedShape.style.strokeStyle.width = width;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setStrokeStyle(pattern: StrokePattern, dashArray?: number[]) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.strokeStyle = { ...this.currentStyle.strokeStyle };
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setOpacity(opacity: number) {
    ...
    this.currentStyle.opacity = validOpacity;
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.opacity = validOpacity;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setGradient(gradient: Gradient | undefined) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.gradient = gradient;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setTextColor(color: string) {
    ...
    if (this.selectedShape && this.selectedShape.type === 'text') {
      this.selectedShape.style = { ...this.selectedShape.style || this.currentStyle, textColor: color };
      // Text shape also has top-level color? No, we use style.textColor mostly now
      (this.selectedShape as any).color = color;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  setDesignColor(color: string) {
    ...
    if (this.selectedShape && this.selectedShape.style) {
      this.selectedShape.style.designColor = color;
      this.broadcastShapeUpdate(this.selectedShape);
      this.clearCanvas();
    }
  }
  ...
  getShapeAtPosition(x: number, y: number): Shape | null {
    ...
    if (this.selectedShape) {
      const handle = this.getResizeHandleAtPosition(x, y);
      if (handle) return this.selectedShape;
    }

    const hitTolerance = 10 / this.scale; // Adjustable tolerance
    ...
    for (let i = this.existingShapes.length - 1; i >= 0; i--) {
      ...
      if (shape.type === "rect" || shape.type === "ellipse" || shape.type === "triangle") {
        ...
        if (dist <= Math.max(5, shape.style?.strokeWidth || 2) / this.scale + hitTolerance) hit = true;
      } else if (shape.type === "path") {
        for (let j = 0; j < shape.points.length - 1; j++) {
          const p1 = shape.points[j];
          const p2 = shape.points[j + 1];
          const dist = this.distancePointToSegment(x, y, p1.x, p1.y, p2.x, p2.y);
          if (dist <= Math.max(6, shape.style?.strokeWidth || 2) / this.scale + hitTolerance) {
            hit = true;
            break;
          }
        }
      } else if (shape.type === "text") {
        const padding = 10; // Extra padding for easier text selection
        const tempCanvas = document.createElement('canvas');
        const tempCtx = tempCanvas.getContext('2d')!;
  ...
  clearCanvas() {
    ...
    validShapes.forEach((shape) => {
      ...
      if (this.selectedShape === shape) {
        // We will enable this later for the selection box only, not the shape stroke itself
        // unless we want a specific effect. For now, keep original stroke but maybe add glow?
        // Actually, let's keep the rendering clean and draw selection ON TOP.
        this.ctx.strokeStyle = style.strokeColor;
        this.ctx.lineWidth = style.strokeWidth;
        this.applyStrokeStyle(this.ctx, style.strokeStyle);
      } else if (this.hoveredShape === shape) {
        this.ctx.shadowColor = "#8B5CF6"; // Purple glow for hover
  ...
  private setSelection(shape: Shape | null) {
    this.selectedShape = shape;
  ...
  mouseMoveHandler = (e: MouseEvent) => {
    ...
    if (this.clicked && !this.isDragging) {
      // Pencil Drawing
      if (this.isDrawingPath) { // Check isDrawingPath for pencil
        const worldPos = this.screenToWorld(x, y);
        this.currentPathPoints.push({ x: worldPos.x, y: worldPos.y });
        this.clearCanvas();
        return;
      ...
      if (this.selectedTool === "rect" || this.selectedTool === "triangle" || this.selectedTool === "ellipse") {
        ...
        if (this.selectedTool === "rect") {
          ...
          // simplified triangle preview
          this.ctx.beginPath();
          this.ctx.moveTo(screenPos.x + screenWidth / 2, screenPos.y);
          this.ctx.lineTo(screenPos.x, screenPos.y + screenHeight);
          this.ctx.lineTo(screenPos.x + screenWidth, screenPos.y + screenHeight);
          this.ctx.closePath();
          this.ctx.stroke();
        }
      } else if (this.selectedTool === "circle") {
    ...
    // 5. Hover State (not clicked)
    const worldPos = this.screenToWorld(x, y);
    const shape = this.getShapeAtPosition(worldPos.x, worldPos.y);

    // Update cursor based on hover
    if (this.isResizing) {
      // Cursor already set in mouseDown
    } else if (this.selectedShape && this.getResizeHandleAtPosition(x, y)) {
      const handle = this.getResizeHandleAtPosition(x, y);
      let cursor = "default";
      ...
      this.canvas.style.cursor = this.selectedTool === "pencil" ? "crosshair" : "default";
  ...
  private getBoundingBox(shape: Shape): { x: number, y: number, width: number, height: number } | null {
    if (shape.type === "rect" || shape.type === "ellipse" || shape.type === "triangle") {
      ...
      tempCtx.font = `${shape.fontSize}px Arial`;
      const metrics = tempCtx.measureText(shape.text);
      return { x: shape.x, y: shape.y, width: metrics.width, height: shape.fontSize * 1.3 };
    }
    return null;
  }
  ...
  private drawSelectionBox(shape: Shape) {
    ...
    const screenPos = this.worldToScreen(boundingBox.x, boundingBox.y);
    const screenWidth = boundingBox.width * this.scale;
    const screenHeight = boundingBox.height * this.scale;

    this.ctx.save();
    // Selection Box - Draw ON TOP of everything
    ...
    handles.forEach(handle => {
      this.ctx.beginPath();
      // Handles are already screen coords
      this.ctx.rect(handle.x, handle.y, handle.width, handle.height);
      this.ctx.fill();
      this.ctx.stroke();
    });
