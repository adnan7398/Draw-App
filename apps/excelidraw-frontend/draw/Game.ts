...
export class Game {
  ...
  touchStartHandler = (e: TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      ...
      if (resizeHandle) {
        ...
        this.resizeStartX = worldPos.x;
        this.resizeStartY = worldPos.y;
        this.originalShape = { ...resizeHandle.shape };
        return;
      }
  ...
  touchMoveHandler = (e: TouchEvent) => {
    e.preventDefault();
    if (e.touches.length === 1 && this.isDrawingShape) {
      const touch = e.touches[0];
      const rect = this.canvas.getBoundingClientRect();
      const x = touch.clientX - rect.left;
      const y = touch.clientY - rect.top;
      ...
      if (this.selectedTool === "pencil" && this.isDrawingPath) {
        const worldPos = this.screenToWorld(x, y);
        this.currentPathPoints.push({ x: worldPos.x, y: worldPos.y });
        this.clearCanvas();
      }
  ...
  touchEndHandler = (e: TouchEvent) => {
    ...
    if (e.touches.length === 0) {
      if (this.isDrawingShape && this.clicked) {
        const rect = this.canvas.getBoundingClientRect();
        ...
        if (this.selectedTool === "pencil" && this.isDrawingPath) {
          ...
          this.historyStack.push([...this.existingShapes]);
          this.existingShapes.push(pathShape);
          this.clearCanvas();
          ...
          this.historyStack.push([...this.existingShapes]);
          this.existingShapes.push(rectShape);
          this.clearCanvas();
          ...
          this.historyStack.push([...this.existingShapes]);
          this.existingShapes.push(lineShape);
          this.clearCanvas();
